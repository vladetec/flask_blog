...
- 🎯 Entry points to execute your program using `python -m <flask_blog>` or `$ flask_blog` with basic CLI argument parsing.
...
---
# flask_blog Flask Application

Awesome flask_blog created by vladetec
...
```bash
git clone https://github.com/vladetec/flask_blog flask_blog
cd flask_blog
make install
```
...
```bash
flask_blog create-db   # run once
flask_blog populate-db  # run once (optional)
flask_blog add-user -u admin -p 1234  # ads a user
flask_blog run
```
